...
export const FormDropdown: FC<FormDropdownProps> = ({ options, value, setValue, placeholder }) => {
  ...
  const handleSelect = useCallback(
    (e: MouseEvent<HTMLButtonElement>) => {
      ...
      setValue(!!e?.currentTarget?.id && e?.currentTarget?.id !== '' ? e?.currentTarget?.id : placeholder);
      setOpen(false);

      const targetCity = e.currentTarget.getAttribute('data-route');
      router.push(`/${targetCity}`);
    },
    [setValue, router, placeholder],
  );
